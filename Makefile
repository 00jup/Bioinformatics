...
init-venv: ## venv 환경 생성 및 패키지 설치 (python 3.10/3.11/3.12/3.13 필요)
	@VENV_PY="$$(command -v python3.11 2>/dev/null || command -v python3.10 2>/dev/null || command -v python3.12 2>/dev/null || command -v python3.13 2>/dev/null)"; \
	if [ -z "$$VENV_PY" ]; then \
		echo "❌ python3.10 / 3.11 / 3.12 / 3.13 중 하나가 필요합니다."; \
		echo "   설치 예시 (macOS): brew install python@3.13"; \
		echo "   설치 예시 (Ubuntu): sudo apt install python3.13 python3.13-venv"; \
		exit 1; \
	fi; \
